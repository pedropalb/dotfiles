...
- `modules/`: Contains the modularized configuration files:
  ...
  - `modules/dev.nix`: Git, lazygit, Neovim, npm environment, and all language toolchains (Rust, Node, Python, Nix, Lua, shell, Docker, markup, TeX, TOML). Declares `my.languages.{haskell,java,kotlin}.enable` opt-in options for Haskell, Java, and Kotlin tooling (off by default; toggled via `extraLanguages` in `flake.nix`).
...
### Coding Agents (herdr and omp)

`herdr` and `omp` are installed through Home Manager from the `llm-agents.nix`
flake input. Update them with:
...
configured before applying this flake on a new machine. Nix ignores
binary-cache settings from a non-root user, and `/etc/nix/nix.conf` is
regenerated by the Determinate Systems installer.

Append these settings as root:

```bash
sudoedit /etc/nix/nix.custom.conf
```
...
Restart the daemon and verify the cache is active:
