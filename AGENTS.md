...
`omp` is installed outside Home Manager with Bun:

```bash
bun install -g @oh-my-pi/pi-coding-agent
```

The Bun global install uses the paths configured in `modules/dev.nix`, placing
the executable in `~/.local/bin` and packages in `~/.local/lib/bun`. Updating
uses the same command. Runtime configuration and sessions remain under
`~/.omp/agent`, independently of the installed executable.
